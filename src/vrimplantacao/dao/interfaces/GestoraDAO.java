...
public class GestoraDAO extends InterfaceDAO {
        ...
        try (Statement stm = ConexaoSqlServer.getConexao().createStatement()) {
            try (ResultSet rst = stm.executeQuery(
                ...
                while (rst.next()) {
                    ...
                    switch (civil) {
                        case "CA": imp.setEstadoCivil(TipoEstadoCivil.CASADO); break;
                        case "SO": imp.setEstadoCivil(TipoEstadoCivil.SOLTEIRO); break;
                        case "VI": imp.setEstadoCivil(TipoEstadoCivil.VIUVO); break;
    ...
    public List<ReceberCreditoRotativoVO> carregarReceberCliente() throws Exception {
        ...
        try (Statement stm = ConexaoSqlServer.getConexao().createStatement()) {
            try (ResultSet rst = stm.executeQuery(
                ...
                while (rst.next()) {
                    ...
                    vo.setDataemissao(rst.getDate("emissao"));
                    vo.setNumerocupom(rst.getInt("cupom"));
                    vo.setValor(rst.getDouble("valor"));
                    vo.setObservacao("IMPORTADO VR" + rst.getString("observacao"));
                    vo.setDatavencimento(rst.getDate("vencimento"));
                    vo.setValorjuros(rst.getDouble("juros"));
                    vo.setCnpjCliente(Utils.stringToLong(rst.getString("cnpj")));
                    vo.setNomeCliente(rst.getString("nome"));
