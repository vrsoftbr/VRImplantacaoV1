...
import vrimplantacao.dao.interfaces.DGComDAO;
import vrimplantacao.vo.loja.LojaVO;

public class DGComGUI extends VRInternalFrame {

    private final DGComDAO importacaoDGCom = new DGComDAO();
    private List<LojaVO> vLojaDestino = new ArrayList<>();
    private final ConexaoSqlServer connSQLServer = new ConexaoSqlServer();

    public DGComGUI(VRMdiFrame i_mdiFrame) throws Exception {
        super(i_mdiFrame);
        initComponents();

        txtHostSqlServer.setText("");
        txtBancoDadosSqlServer.setText("");
        txtUsuarioSqlServer.setText("");
        txtSenhaSqlServer.setText("");
            ...
            public void run() {
                ...
                try {
                    ...
                    if ((!txtArquivoBalanca.getArquivo().isEmpty())
                        ...
                        importacaoDGCom.importarProdutoBalanca(txtArquivoBalanca.getArquivo(), opcao);
                    }

                    ProgressBar.dispose();

                    Util.exibirMensagem("Importação arquivo balança realizada com sucesso!", getTitle());
            ...
            public void run() {
                try {
                    ...
                    if (chkMercadologico.isSelected()) {
                        importacaoDGCom.importarMercadologico();
                    }
                    
                    if (chkProduto.isSelected()) {
                        if (chkManterBalanca.isSelected()) {                            
                            importacaoDGCom.importarProdutoManterBalanca(id_loja, id_lojaCliente);
                        } else {
                            importacaoDGCom.importarProduto(id_loja, id_lojaCliente);
                        }                        
                    }

                    if (chkCustoProduto.isSelected()) {
                        importacaoDGCom.importarCustoProduto(id_loja, id_lojaCliente);                        
                    }
                    
                    if (chkPrecoProduto.isSelected()) {
                        importacaoDGCom.importarPrecoProduto(id_loja, id_lojaCliente);
                    }
                    
                    if (chkEstoqueProduto.isSelected()) {
                        importacaoDGCom.importarEstoqueProduto(id_loja, id_lojaCliente, chkSomarEstoque.isSelected());
                    }
                    
                    if (chkCodigoBarras.isSelected()) {
                        importacaoDGCom.importarCodigoBarra(id_lojaCliente, id_loja);
                    }
                    
                    if (chkProdutoSemEAN.isSelected()) {
                        importacaoDGCom.importarCodigoBarraEmBranco();
                    }
                    
                    if (chkFabricante.isSelected()) {
                        importacaoDGCom.importarIdFornecedorFabricante();
                    }
                    
                    if (chkFornecedor.isSelected()) {
                        importacaoDGCom.importarFornecedor();
                    }
                    
                    if (chkProdutoFornecedor.isSelected()) {
                        importacaoDGCom.importarProdutoFornecedor(id_loja);
                    }
                    
                    if (chkClientePreferencial.isSelected()) {
                        importacaoDGCom.importarClientePreferencial(id_loja, id_lojaCliente);
                    }
                    
                    ProgressBar.dispose();
                    Util.exibirMensagem("Importação DGCom realizada com sucesso!", getTitle());
                } catch (Exception ex) {
                    try {
                        connSQLServer.close();
                    } catch (Exception ex1) {
    ...
    private void initComponents() {
        ...
        vRLabel8.setText("Tipo do documento no DGCom");
        ...
        vRLabel9.setText("Tipo do documento no DGCom");
