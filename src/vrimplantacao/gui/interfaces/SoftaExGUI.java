...
import vrimplantacao.dao.interfaces.SoftaExDAO;
import vrimplantacao.dao.interfaces.VRSoftwareDAO;
import vrimplantacao.vo.loja.LojaVO;

public class SoftaExGUI extends VRInternalFrame {

    private VRSoftwareDAO migracaoVRDAO = new VRSoftwareDAO();
    private SoftaExDAO importacaoSoftaExDAO = new SoftaExDAO();
    ...
    public SoftaExGUI(VRMdiFrame i_mdiFrame) throws Exception {
        super(i_mdiFrame);
        initComponents();

        txtHostFirebird.setText("127.0.0.1");
        txtBancoDadosFirebird.setArquivo("C:\\Dados\\Dnutri\\SoftaEX\\dados\\SOFTAEX.IB");
            ...
            public void run() {
                ...
                try {
                    ...
                    if ((!txtArquivoBalanca.getArquivo().isEmpty()) &&
                        ...
                        importacaoSoftaExDAO.importarProdutoBalanca(txtArquivoBalanca.getArquivo(), opcao);
            ...
            public void run() {
                try {
                    ...
                    if (chkFamiliaProduto.isSelected()) {                        
                        importacaoSoftaExDAO.importarFamiliaProdutoSoftaEx();
                    }
                    
                    if (chkMercadologico.isSelected()) {
                        importacaoSoftaExDAO.importarMercadologicoSoftaEx();
                    }
                    
                    if (chkProduto.isSelected()) {
                        importacaoSoftaExDAO.importarProdutoSoftaEx(id_loja);                        
                    }
                    
                    if (chkCustoProduto.isSelected()) {
                        importacaoSoftaExDAO.importarCustoProdutoSoftaEx(id_loja, id_lojaCliente);
                    }                    
                    
                    if (chkPrecoProduto.isSelected()) {
                        importacaoSoftaExDAO.importarPrecoProdutoSoftaEx(id_loja, id_lojaCliente);
                    }                                        
                    
                    if (chkEstoqueProduto.isSelected()) {
                        importacaoSoftaExDAO.importarEstoqueProdutoSoftaEx(id_loja, id_lojaCliente);
                    }                      
                    
                    if (chkFornecedor.isSelected()) {
                        importacaoSoftaExDAO.importarFornecedorSoftaEx();
                    } 
                    
                    if (chkProdutoFornecedor.isSelected()) {
                        importacaoSoftaExDAO.importarProdutoFornecedorSoftaEx();
                    }

                    if (chkClientePreferencial.isSelected()) {
                        importacaoSoftaExDAO.importarClientePreferencialSoftaEx(id_loja, id_lojaCliente);
                    } 
                    ...
                    if (chkCodigobarras.isSelected()) {
                        importacaoSoftaExDAO.importarCodigoBarraSoftaEx();
                    }

                    ProgressBar.dispose();
                    Util.exibirMensagem("Importação SoftaEx realizada com sucesso!", getTitle());
                } catch (Exception ex) {
                    try {                    
                        connFB.close();
                    } catch (Exception ex1) {
