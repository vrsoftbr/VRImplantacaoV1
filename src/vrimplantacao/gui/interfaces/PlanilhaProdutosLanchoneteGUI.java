...
import vrimplantacao.dao.interfaces.PlanilhaProdutosLanchoneteDAO;
import vrimplantacao.vo.loja.LojaVO;

public class PlanilhaProdutosLanchoneteGUI extends VRInternalFrame {

    public PlanilhaProdutosLanchoneteGUI(VRMdiFrame i_mdiFrame) throws Exception {
        super(i_mdiFrame);
        initComponents();
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                try {
                    ...
                    if (!flcArquivo.getArquivo().isEmpty()) {
                        new PlanilhaProdutosLanchoneteDAO().importarProdutosLanchonete(flcArquivo.getArquivo().trim());
                        Util.exibirMensagem("Migração Produtos Lanchonete realizada com sucesso!", getTitle());
                    } else {
                        Util.exibirMensagem("Selecione o arquivo", getTitle());
                    }
                    ProgressBar.dispose();
    ...
    private void initComponents() {
        ...
        vRLabel1.setText("Planilha Produtos Lanchonete");
