...
import vrimplantacao.dao.interfaces.MobilityDAO;
import vrimplantacao.vo.loja.LojaVO;

public class MobilityGUI extends VRInternalFrame {

    private MobilityDAO importacaoMobilityDAO = new MobilityDAO();
    ...
    public MobilityGUI(VRMdiFrame i_mdiFrame) throws Exception {
        super(i_mdiFrame);
        initComponents();
        
        txtHostFirebird.setText("127.0.0.1");
        txtBancoDadosFirebird.setArquivo("C:\\Dados\\Colibri\\BANCO.FDB");
            ...
            public void run() {
                ...
                try {
                    ...
                    if ((!txtArquivoBalanca.getArquivo().isEmpty()) &&
                        ...
                        importacaoMobilityDAO.importarProdutoBalanca(txtArquivoBalanca.getArquivo(), opcao);
                        ProgressBar.dispose();
                        Util.exibirMensagem("Importação Mobility Realizada com Sucesso!", getTitle());
                    }
                } catch(Exception ex) {
            ...
            public void run() {
                try {
                    ...
                    if (chkFamiliaProduto.isSelected()) {                        
                        importacaoMobilityDAO.importarFamiliaProdutoMobility();
                    }
                    
                    if (chkMercadologico.isSelected()) {
                        importacaoMobilityDAO.importarMercadologicoMobility();
                    }
                    
                    if (chkProduto.isSelected()) {
                        importacaoMobilityDAO.importarProdutoMobility(id_loja);
                    }
                    
                    if (chkCustoProduto.isSelected()) {
                        importacaoMobilityDAO.importarCustoProdutoMobility(id_loja, id_lojaCliente);
                    }                    
                    
                    if (chkPrecoProduto.isSelected()) {
                        importacaoMobilityDAO.importarPrecoProdutoMobility(id_loja, id_lojaCliente);
                    }                                        
                    
                    if (chkEstoqueProduto.isSelected()) {
                        importacaoMobilityDAO.importarEstoqueProdutoMobility(id_loja, id_lojaCliente);
                    }                      
                    
                    if (chkFornecedor.isSelected()) {
                        importacaoMobilityDAO.importarFornecedorMobility();
                    } 
                    
                    if (chkProdutoFornecedor.isSelected()) {
                        importacaoMobilityDAO.importarProdutoFornecedorMobility();
                    }

                    if (chkClientePreferencial.isSelected()) {
                        importacaoMobilityDAO.importarClientePreferencialMobility(id_loja, id_lojaCliente);
                    } 
                    
                    if (chkCreditoRotativo.isSelected()) {
                        //importacaoMobilityDAO.importarReceberClienteMobility(id_loja, id_lojaCliente);
                    }                    
                    
                    if (chkChequeReceber.isSelected()) {
                        //importacaoMobilityDAO.importarChequeReceber(id_loja, id_lojaCliente);
                    }
                    
                    if (chkClienteDuplicado.isSelected()) {
                        //importacaoMobilityDAO.corrigirClienteDuplicado();
                    }
                    
                    if (chkCodigobarras.isSelected()) {
                        importacaoMobilityDAO.importarCodigoBarra();
                    }
