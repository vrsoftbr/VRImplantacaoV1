...
import vrimplantacao.dao.interfaces.WisaSoftDAO;
import vrimplantacao.dao.interfaces.VRSoftwareDAO;
import vrimplantacao.vo.loja.LojaVO;

public class WisaSoftGUI extends VRInternalFrame {

    private VRSoftwareDAO migracaoVRDAO = new VRSoftwareDAO();
    private WisaSoftDAO importacaoWisaSoftDAO = new WisaSoftDAO();
    ...
    public WisaSoftGUI(VRMdiFrame i_mdiFrame) throws Exception {
        super(i_mdiFrame);
        initComponents();
        
        txtHostFirebird.setText("127.0.0.1");
        txtBancoDadosFirebird.setArquivo("C:\\Dados\\VarejaoMarques\\RETAGUARDA_WISA.FB");
            ...
            public void run() {
                ...
                try {
                    ...
                    if ((!txtArquivoBalanca.getArquivo().isEmpty()) &&
                        ...
                        importacaoWisaSoftDAO.importarProdutoBalanca(txtArquivoBalanca.getArquivo(), opcao);
                        ProgressBar.dispose();
                        Util.exibirMensagem("Importação WisaSoft Realizada com Sucesso!", getTitle());
                    }
                } catch(Exception ex) {
            ...
            public void run() {
                try {
                    ...
                    if (chkFamiliaProduto.isSelected()) {                        
                        //importacaoWisaSoftDAO.importarFamiliaProdutoWisaSoft();
                    }
                    
                    if (chkMercadologico.isSelected()) {
                        //importacaoWisaSoftDAO.importarMercadologicoWisaSoft();
                    }
                    
                    if (chkProduto.isSelected()) {
                        importacaoWisaSoftDAO.importarProdutoWisaSoft(id_loja);
                    }
                    
                    if (chkCustoProduto.isSelected()) {
                        importacaoWisaSoftDAO.importarCustoProdutoWisaSoft(id_loja, id_lojaCliente);
                    }                    
                    
                    if (chkPrecoProduto.isSelected()) {
                        importacaoWisaSoftDAO.importarPrecoProdutoWisaSoft(id_loja, id_lojaCliente);
                    }                                        
                    
                    if (chkEstoqueProduto.isSelected()) {
                        importacaoWisaSoftDAO.importarEstoqueProdutoWisaSoft(id_loja, id_lojaCliente);
                    }                      
                    
                    if (chkFornecedor.isSelected()) {
                        importacaoWisaSoftDAO.importarFornecedorWisaSoft();
                    } 
                    
                    if (chkProdutoFornecedor.isSelected()) {
                        importacaoWisaSoftDAO.importarProdutoFornecedorWisaSoft();
                    }

                    if (chkClientePreferencial.isSelected()) {
                        importacaoWisaSoftDAO.importarClientePreferencialWisaSoft(id_loja, id_lojaCliente);
                    } 
                    
                    if (chkCreditoRotativo.isSelected()) {
                        importacaoWisaSoftDAO.importarReceberClienteWisaSoft(id_loja, id_lojaCliente);
                    }                    
                    
                    if (chkChequeReceber.isSelected()) {
                        importacaoWisaSoftDAO.importarChequeReceberWisaSoft(id_loja, id_lojaCliente);
                    }
                    
                    if (chkClienteDuplicado.isSelected()) {
                        //importacaoWisaSoftDAO.corrigirClienteDuplicado();
                    }
                    
                    if (chkPisCofins.isSelected()) {
                        importacaoWisaSoftDAO.importarPisCofinsProdutoWisa();
                    }
    ...
    private void initComponents() {
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(vRPanel2, javax.swing.GroupLayout.PREFERRED_SIZE, 138, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
