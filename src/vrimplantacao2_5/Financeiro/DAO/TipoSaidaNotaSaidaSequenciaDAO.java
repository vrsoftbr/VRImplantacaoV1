 ...
 * @author Bruno
 */
public class TipoSaidaNotaSaidaSequenciaDAO {

    private int idLojaVR = 1;
    private String importSistema = null;
    private String importLoja = null;

    private static Logger LOG = Logger.getLogger(TipoSaidaNotaSaidaSequenciaDAO.class.getName());

    public void salvar(TipoSaidaNotaFiscalSequenciaVO vo) throws Exception {
        try (Statement stm = Conexao.createStatement()) {
            SQLBuilder sql = new SQLBuilder();

            sql.setSchema("public");
            sql.setTableName("tiposaidanotasaidasequencia");

            sql.put("id", vo.getId());
            sql.put("id_loja", vo.getId_loja());
            sql.put("id_tiposaida", vo.getId_tipoSaida());
            sql.put("id_notasaidasequencia", vo.getId_notaSaidaSequencia());
