...
public class TipoModeloDAO {
    ...
    private static Logger LOG = Logger.getLogger(TipoModeloDAO.class.getName());

    public void salvar(TipoModeloVO vo) throws Exception {
        try (Statement stm = Conexao.createStatement()) {
            SQLBuilder sql = new SQLBuilder();

            sql.setSchema("pdv");
            sql.setTableName("tipomodelo");

            sql.put("id", vo.getId());
            sql.put("id_tipomarca", vo.getId_tipoMarca());
            sql.put("descricao", vo.getDescricao());
            sql.put("codigom", vo.getCodigoM());
            sql.put("sat", vo.isSat());
