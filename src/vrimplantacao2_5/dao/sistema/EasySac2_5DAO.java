 ...
 * @author Alan
 */
public class EasySac2_5DAO extends InterfaceDAO implements MapaTributoProvider {
        ...
        try (Statement stm = ConexaoSqlServer.getConexao().createStatement()) {
            try (ResultSet rst = stm.executeQuery(
                ...
                while (rst.next()) {
                    ...
                    if (rst.getDouble("margem") > 2234){
                        imp.setMargem(0);
                    } else imp.setMargem(Math.round(rst.getDouble("margem") * 100.0) / 100.0);
