...
public class MercadologicoAnteriorDAO {
    ...
    public String getCodMerc1(String sistema, String loja, String merc2) throws Exception {
        try (Statement stm = Conexao.createStatement()) {
            try (ResultSet rst = stm.executeQuery(
                    "select distinct ant_merc1 "
                    + "from implantacao.codant_mercadologico "
                    + "where imp_sistema = '" + sistema + "' "
                    + "and imp_loja = '" + loja + "' "
                    + "and ant_merc2 = '" + merc2 + "'"
            )) {
                if (rst.next()) {
                    return rst.getString("ant_merc1");
                } else {
                    return null;
                }
            }
