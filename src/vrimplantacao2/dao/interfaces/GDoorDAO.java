        ...
        try(Statement stm = ConexaoFirebird.getConexao().createStatement()) {
            try(ResultSet rs = stm.executeQuery(
                ...
                while(rs.next()) {
                    ...
                    if("Balança".equals(rs.getString("grupo").trim())) {
                        if(rs.getString("ean") != null && rs.getString("ean").length() < 7) {
                            String ean = rs.getString("ean");
                            ean = ean.substring(2, ean.length());
                            imp.setEan(ean);
                            imp.setImportId(ean);
                    ...
                    imp.setIcmsCst(rs.getString("icmscst"));
                    imp.setIcmsCstConsumidor(rs.getString("icmscst"));
                    imp.setIcmsCstEntrada(rs.getInt("icmscst"));
