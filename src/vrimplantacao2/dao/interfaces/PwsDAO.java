...
public class PwsDAO extends InterfaceDAO implements MapaTributoProvider {
    ...
    public List<FamiliaProdutoIMP> getFamiliaProduto() throws Exception {
        ...
        try (Statement stm = ConexaoFirebird.getConexao().createStatement()) {
            try (ResultSet rst = stm.executeQuery(
                    "select distinct\n" +
                    "    p.cod_princ_bm_pro id_familia,\n" +
                    "    trim(f.ds_pro) descricao\n" +
                    "from\n" +
                    "    produto p\n" +
                    "    join produto f on p.cod_princ_bm_pro = f.cd_pro\n" +
                    "where\n" +
                    "    not nullif(trim(p.cod_princ_bm_pro),'') is null\n" +
                    "order by\n" +
                    "    1"
            )) {
                while (rst.next()) {
        ...
        try (Statement stm = ConexaoFirebird.getConexao().createStatement()) {
            try (ResultSet rst = stm.executeQuery(
                ...
                while (rst.next()) {
                    ...
                    double red = Utils.stringToDouble(rst.getString("vl_aliquota_redz_pro"), 0.00);
    ...
    public List<ProdutoIMP> getProdutos() throws Exception {
        ...
        try (Statement stm = ConexaoFirebird.getConexao().createStatement()) {
            try (ResultSet rst = stm.executeQuery(
                    "select\n" +
                    "    p.cd_pro id,\n" +
                    "    p.dh_inclusao_pro datacadastro,\n" +
                    "    p.cd_pro ean,\n" +
                    "    1 as qtdembalagem,\n" +
                    "    p.un_pro unidade,\n" +
                    ...
                    "    p.idsfam_pro mercadologico,\n" +
                    "    coalesce(nullif(trim(p.cod_princ_bm_pro),''), p.cd_pro) id_familia,\n" +
                    "    coalesce(p.qt_est_min_pro, 0) estoqueminimo,\n" +
                    "    coalesce(p.qt_est_ideal_pro, 0) estoquemaximo,\n" +
                    "    coalesce(p.qt_est_atu_pro, 0) estoque,\n" +
                    "    coalesce(p.markup_pro, 0) margem,\n" +
                    "    coalesce(p.vl_com_pro, 0) custocomimposto,\n" +
                    "    coalesce(p.vl_com_bruto_pro, 0) custosemimposto,\n" +
                    "    coalesce(p.vl_ven_pro, 0) preco,\n" +
