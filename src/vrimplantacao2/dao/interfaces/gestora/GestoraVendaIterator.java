...
public class GestoraVendaIterator extends MultiStatementIterator<VendaIMP> {
    ...
    public GestoraVendaIterator(String idLoja, Date dataInicial, Date dataTermino) {        
        super(
    ...
    private String getFullSQL(int idLoja, SQLUtils.Intervalo intervalo) {
        ...
        try (Statement st = ConexaoSqlServer.getConexao().createStatement()) {
            try (ResultSet rs = st.executeQuery(
                    "select * from information_schema.TABLES t where TABLE_NAME = '" + getNomeTabela(intervalo.dataInicial) + "nf'"
            )) {
                nfExiste = rs.next();
            }
        } catch (Exception ex) {
        ...
        if (nfExiste) {
            return
                ...
                "	) cx on\n" +
                "		v.caixa = cx.caixa\n" +
                "	left join " + getNomeTabela(intervalo.dataInicial) + "nf nf on\n" +
    ...
    private static class GestoraNextBuilder implements MultiStatementIterator.NextBuilder<VendaIMP> {
    ...
    private static class LinearStatementBuilder implements MultiStatementIterator.StatementBuilder {
        @Override
